...
import { CheckCircle, FileText, Award, User, Heart, Globe, Star, Book, CreditCard, Loader, UserCheck, Camera, Users, Shield, Phone, Mail, MessageCircle } from "lucide-react";
import CountriesRequirementBackground from "@/assets/Countries_requirement_background.jpg";

// Use a placeholder passport icon
...
const stepIcons = [
  ...
  <Award className="w-5 h-5" />,          // Offer Letter & CoE
  <PassportIcon className="w-5 h-5" />,   // Passport
  <CreditCard className="w-5 h-5" />,     // Financial Proof
  <Book className="w-5 h-5" />,           // Academic Records
  <Globe className="w-5 h-5" />,          // English Proficiency
  <User className="w-5 h-5" />,           // GTE Statement
  <Heart className="w-5 h-5" />,          // OSHC
  <CreditCard className="w-5 h-5" />,     // Proof of Payment
  <Star className="w-5 h-5" />,           // SOP/Resume
  <Loader className="w-5 h-5" />,         // Medical Examination
  <Shield className="w-5 h-5" />,         // Character Documents
  <Users className="w-5 h-5" />,          // Relationship Proof
  <Camera className="w-5 h-5" />,         // Photograph
];
...
const requirements = [
  {
    ...
    documents: [
      { name: "Form 157A", details: "Online application through ImmiAccount" }
    ]
  },
  {
    category: "Offer Letter & CoE",
    documents: [
      { name: "Confirmation of Enrolment (CoE)", details: "Issued by the university/college after acceptance" }
    ]
  },
  {
    category: "Passport",
    documents: [
      { name: "Valid Passport", details: "Must be valid for your entire study period" }
    ]
  },
  {
    category: "Financial Proof",
    documents: [
      { name: "Bank Statements", details: "Show funds for tuition + living + travel" },
      { name: "Sponsor Letter / Affidavit of Support", details: "If funds come from parents or sponsor" },
      { name: "Education Loan Letter", details: "From recognized financial institution" }
    ]
  },
  {
    category: "Academic Records",
    documents: [
      { name: "Mark Sheets, Certificates, Degrees", details: "10th, 12th, Bachelor's, or Master's (as applicable)" }
    ]
  },
  {
    category: "English Proficiency",
    documents: [
      { name: "IELTS / PTE / TOEFL", details: "Minimum usually 5.5 – 6.5 depending on program" }
    ]
  },
  {
    category: "GTE Statement",
    documents: [
      { name: "Genuine Temporary Entrant (GTE)", details: "Personal statement explaining intent to study and return home" }
    ]
  },
  {
    category: "OSHC",
    documents: [
      { name: "Overseas Student Health Cover", details: "Mandatory health insurance for entire stay" }
    ]
  },
  {
    category: "Proof of Payment",
    documents: [
      { name: "Fee Receipt", details: "Tuition and OSHC payment receipts" }
    ]
  },
  {
    category: "SOP / Resume",
    documents: [
      { name: "Statement of Purpose", details: "Explains study goals and background" }
    ]
  },
  {
    category: "Medical Examination",
    documents: [
      { name: "Health Check Report", details: "From an Australian panel doctor" }
    ]
  },
  {
    category: "Character Documents",
    documents: [
      { name: "Police Clearance Certificate", details: "From all countries lived in for >12 months" }
    ]
  },
  {
    category: "Relationship Proof (if dependents)",
    documents: [
      { name: "Marriage Certificate, Birth Certificate", details: "If applying with spouse/children" }
    ]
  },
  {
    category: "Photograph",
    documents: [
      { name: "Recent Passport-Size Photo", details: "As per Australian visa specifications" }
    ]
  }
];
      ...
      <main className="flex-grow py-12">
        <div className="container mx-auto px-2 sm:px-6 lg:px-8 max-w-5xl">
          ...
          >
            <div className="text-center mb-8">
              ...
              <p className="text-lg text-slate-500">
                Complete document checklist for Australian student visa (Subclass 500)
              </p>
            </div>
          </motion.div>
          <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="rounded-2xl shadow-xl border-2 border-blue-200/60 bg-white/70 backdrop-blur-md h-full flex flex-col justify-center px-2 py-6">
                  ...
                  <div className="flex md:flex-col flex-row gap-4 md:gap-0 justify-center md:items-stretch items-center relative z-10">
                    <div className="relative flex md:flex-col flex-row md:gap-0 gap-4 w-full">
                      {/* Animated vertical line behind steps */}
            ...
            {/* Right: Animated Content - PREVIOUS MODERN FLOATING CARD DESIGN */}
