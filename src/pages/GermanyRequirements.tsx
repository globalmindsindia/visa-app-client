...
const requirements = [
  {
    ...
    documents: [
      { name: "National Visa Application", details: "Completed and signed application form" },
      { name: "Declaration", details: "Additional declaration forms as required" }
    ]
  },
  {
    category: "University Documents",
    documents: [
      { name: "Letter of Admission", details: "From German university or preparatory course" },
      { name: "Proof of University Application", details: "If admission is still pending" }
    ]
  },
  {
    ...
    documents: [
      ...
      { name: "Passport Photos", details: "Biometric passport photos (35x45mm)" }
    ]
  },
  {
    category: "Financial Proof",
    documents: [
      { name: "Blocked Account (Sperrkonto)", details: "€11,208 per year in blocked account" },
      { name: "Scholarship Certificate", details: "If receiving scholarship funding" },
      { name: "Bank Statements", details: "Parents' financial statements if sponsored" },
      { name: "Sponsor Declaration", details: "Verpflichtungserklärung from German resident" }
    ]
  },
  {
    category: "Academic Qualifications",
    documents: [
      { name: "Educational Certificates", details: "School leaving certificates and transcripts" },
      { name: "University Degree", details: "Bachelor's degree for Master's programs" },
      { name: "Recognition Certificate", details: "From uni-assist or university if required" }
    ]
  },
  {
    category: "Language Proficiency",
    documents: [
      { name: "German Language Certificate", details: "TestDaF, DSH, or Goethe certificate" },
  ...
  },
  {
    ...
    documents: [
      { name: "Health Insurance", details: "Proof of health insurance coverage in Germany" },
  ...
  },
  {
    ...
    documents: [
      { name: "Motivation Letter", details: "Statement of purpose for studies" },
      { name: "CV/Resume", details: "Detailed curriculum vitae" },
      { name: "APS Certificate", details: "Academic evaluation from APS (for certain countries)" }
    ]
  }
];
        ...
        <div className="container mx-auto px-2 sm:px-6 lg:px-8 max-w-5xl">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-8">
            <div className="text-center mb-8">
              ...
              <p className="text-lg text-slate-500">Complete document checklist for German student visa application</p>
            </div>
          </motion.div>
                  ...
                  <div className="flex md:flex-col flex-row gap-4 md:gap-0 justify-center md:items-stretch items-center relative z-10">
                    <div className="relative flex md:flex-col flex-row md:gap-0 gap-4 w-full">
                      ...
                      {requirements.map((cat, idx) => {
                        ...
                        return (
                          <motion.button
                            layout key={cat.category} onClick={() => handleCategoryChange(idx)}
                            ...
                            whileHover={{ scale: selectedCategory === idx ? 1.06 : 1.03 }} whileTap={{ scale: 0.98 }}
                          >
                            <div className="relative flex-shrink-0">
                              <motion.div
                                className={`w-12 h-12 flex items-center justify-center rounded-full shadow-md border-4 border-white transition-all ${selectedCategory === idx ? `bg-gradient-to-br ${gradient} shadow-xl` : "bg-white"}`}
                                animate={{ boxShadow: selectedCategory === idx ? "0 4px 18px 0 rgba(56,189,248,0.15)" : "0 1px 3px 0 rgba(0,0,0,0.06)" }}
              ...
              <div className="rounded-2xl shadow-xl border border-slate-100 bg-gradient-to-br from-blue-50 via-white to-amber-50 p-8 flex flex-col items-center text-center space-y-5">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center items-center w-full mt-2">
                  ...
                  <Button variant="outline" className="bg-white shadow px-6 text-base font-semibold flex items-center gap-2 border min-w-[180px]" onClick={() => window.open('tel:+917353446655')}>
                    <span role="img" aria-label="expert" className="text-lg">💡</span>Talk to Expert
                  </Button>
                </div>
              </div>
            </div>
